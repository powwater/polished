...
apppath <- fs::path_package("powpolished", "examples/test_app")
...
test_that("test creation of deps.yaml", {
  ...
  withr::with_dir(app, {
    ...
    deps_list <- powpolished:::get_package_deps(app_dir = app)

    testthat::expect_equal(deps_list, yaml::read_yaml(fs::path_package("powpolished", "testfiles/deps_no_polished.yaml")))
